...
nulow, nuhigh = float(0.04),float(0.25)
...
#        plt.xlabel(r'$\nu$ (MHz)',fontsize=16)
        ...
        plt.xlim(1000*nurange[0],250)
        
        maxind = np.array(observedflux).argmax()
        print 'MHz  Ratio'
        print 1000*nurange[maxind], Dsval/Dval

            
#Set common x-label
plt.text(145, -0.05, r'$\nu$ (MHz)', ha='center', va='center', fontsize = 20)

#filename = 'FluxDifferentDDs_'+str(nulow)+'_'+str(nuhigh)+'.png'
...
#
#resposAni = modelpow.fit(obsposAni,x=nurange[0:maxindexAni-skipA2],K=0.001,k=-2.5)
#resnegAni = modelpow.fit(obsnegAni,x=nurange[maxindexAni+skipA:len(observedfluxAni)],K=0.001,k=0.1)
#
...
##plt.plot(nurangeMHz[maxindexAni+30:len(observedfluxAni)],neganiK*np.power(nurange[maxindexAni+30:len(observedfluxAni)],-neganialf),'k-.',linewidth = 2.0, label=r'$\gamma_{aniso}$ = %.1f' % -resnegAni.best_values['k'])
##plt.title('Flux spectrum')
#plt.xlabel(r"$\nu$ (GHz)")
#plt.ylabel("Flux (normalised to a max intrinsic flux of 1.0)")
#plt.xlim(40,250)
##plt.legend(loc = 'best',fontsize=11)
#plt.text(80,0.33,r'$\sigma_x = %d$mas and $\sigma_y = %d$mas at 1 GHz ' % (k1,k2), fontsize=12)
#plt.text(120,0.19,r'$\sigma_{x,y} = %d$mas at 1 GHz ' % k1, fontsize=12)
#plt.text(120,0.21,'isotropic',fontsize=16)
#plt.text(80,0.35,'anisotropic',fontsize=16)
#
#
##plt.figure()
